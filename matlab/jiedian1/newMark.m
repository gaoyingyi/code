...
%% 参数
    ...
    c0=1/(alpha*deltaT^2);c1=delta/(alpha*deltaT);c2=1/(alpha*deltaT);
    c3=1/(2*alpha)-1;c4=delta/alpha-1;c5=deltaT/2*(delta/alpha-2);
    c6=deltaT*(1-delta);c7=delta*deltaT; 
...
%%  最简单的初始条件 
    Q0=F(0);                      
    sumQ = zeros(m,n);           sumQ(:,1)=Q0;   %初始外载荷
    U0=zeros(m,1);                     U = zeros(m,n);     U(:,1)=U0;   %初始位移场
    UD0=zeros(m,1);                   UD = zeros(m,n);   UD(:,1)=UD0;   %初始速度场
    UDD0=M\(Q0-C*UD0-K*U0);UDD = zeros(m,n); UDD(:,1)=UDD0;   %初始加速度场
    %% 参数计算量，不需要改变，自动计算
    for i = 1:n-1                                                       %按时间步长循环
